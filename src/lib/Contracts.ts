...
export default class Contracts {
  ...
  public async postContractData(): Promise<ContractsResponse> {
    // Validate body data
    const result: ContractsResponse = this._validateBodyData('post');
    if (!result.success) {
      return result;
    }

    // Format data for SQL statement
    const values: any[] = [];
    const insertFieldsSql: string = this._getInsertFieldsSql(values);
    if (isEmpty(insertFieldsSql)) {
      this._contractsResponse.success = false;
      this._contractsResponse.data = `Invalid or missing fields/values in fields SQL statement`;
      return this._contractsResponse;
    }
  ...
  public async putContractData(): Promise<ContractsResponse> {
    ...
    // Validate body data
    const result: ContractsResponse = this._validateBodyData('put');
    if (!result.success) {
      return result;
    }
    ...
    try {
      ...
      const values: any[] = [id];
      const setSql: string = this._getSetSql(values);
      if (isEmpty(setSql)) {
        this._contractsResponse.success = false;
  ...
  /**
   * Validate the POST or PUT body data
   * @param {string} method    http method so we know what fields to check
   * @returns {ContractsResponse}
   */
  private _validateBodyData(method: string): ContractsResponse {
    // Default response object
    const contractsResponse: ContractsResponse = {
      success: true,
      data: ''
    };
    ...
    if (isEventEmpty(this._event, '$.body')) {
      contractsResponse.success = false;
      contractsResponse.data = `Missing body data in ${method}`;
    ...
    // Convert body fields to snake case
    const objConvertToSnake = {};
    for (let key in body) {
      objConvertToSnake[camelToSnake(key)] = body[key];
    }

    // Validate body data based upon a post or put method
    try {
      switch (method) {
        case 'post':
        ...
        case 'put':
          assert.isSchemaValid(
            'Contract',
            objConvertToSnake,
            schemas.ContractUpdateSchema
          );
          break;
        default:
          contractsResponse.success = false;
          contractsResponse.data = `Unknown method when validating body data: ${method}`;
          return contractsResponse;
      }
    } catch (error) {
      contractsResponse.success = false;
      contractsResponse.data = error.message;
      return contractsResponse;
    }
    return contractsResponse;
  }
  ...
  /**
   *  Extracts/validates put input from body, returns SET SQL statement
   * @param {any[]} values     Array of values
   * @returns {string}         SQL UPDATE SET
   */
  private _getSetSql(values: any[]): string {
    // Return if no event body
    if (isEventEmpty(this._event, '$.body')) {
      return '';
    }
  ...
  /**
   *  Extracts/validates post input from body, returns fields SQL statement
   * @param {any[]} values     Array of values
   * @returns {string}         SQL INSERT fields
   */
  private _getInsertFieldsSql(values: any[]): string {
    // Return if no event body
    if (isEventEmpty(this._event, '$.body')) {
      return '';
    }
    ...
    // Build/return set statements, field1, field2...fieldx
    return fields
      .map((value, index) => {
        return `${value}`;
      })
